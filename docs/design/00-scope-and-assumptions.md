...
- **2026-09-17 (Phase 8):** `SHIPPED` without a `trackingNumber` gives `400 VALIDATION_FAILED` (doc 03 allowed 400 or 422). Moving a shipment to `CANCELLED` through the staff endpoint gives `409 INVALID_STATE_TRANSITION` (cancellation is order-level, Phase 9). The customer is notified once per shipment for `SHIPPED` and `DELIVERED`.
- **2026-09-17 (Phase 9):** Refund issuance is centralised in `payment/RefundService` (invariant `sum(refunds) ≤ payment`, `RefundIssuedEvent`); cancellation refunds `payment − already refunded`, which is the full amount in practice since returns need a delivered order. A cancellation with nothing refundable (never the case today) simply skips the refund.
- **2026-09-17 (Phase 10):** Duplicate `orderItemId`s in a return request give `400 VALIDATION_FAILED` (doc 09 says so); an item that does not belong to the order gives `422 RETURN_NOT_ALLOWED`. `receive` with a wrong or incomplete item list gives `400`, on a non-APPROVED return `409 INVALID_STATE_TRANSITION`. A `PARTIALLY_RETURNED → PARTIALLY_RETURNED` transition is recorded in the history on every further partial return (doc 04 allows the self-loop).
- **2026-09-17 (Phase 10):** Warehouse return listing also accepts `warehouseId` for admins (mirrors the shipment listing); staff are pinned to their own warehouse.
- **2026-09-18 (Phase 12):** Verified once against Neon (PostgreSQL 18): all 10 migrations applied unchanged, the seeder ran, and the full walkthrough (split checkout, fulfilment, return with refund, cancellation, admin filters) completed with no errors. No SQL portability changes were needed.
- **2026-09-18 (Phase 12):** Removed unused code: `ProductService.requireActive` and `BaseEntity.isNew()`. `Money` keeps its small symmetric helper set (`min`/`max`, `of`) as a tested utility.
- **2026-09-18 (Phase 12):** The reconciliation job for lost after-commit events remains future work (README "Out of scope & known limitations").
